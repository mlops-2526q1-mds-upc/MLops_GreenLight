...
# Runs Black (check), Ruff (lint), and Pylint on the mlops_greenlight folder
...
# Consider Black PASS if no files need reformatting
if [[ ${BLACK_NEEDS} -eq 0 ]]; then
  BLACK_RESULT="PASS"
else
  BLACK_RESULT="FAIL"
fi
echo "Black: ${BLACK_RESULT} — files needing format: ${BLACK_NEEDS}"
...
# Consider Ruff PASS if no issues were found
if [[ ${RUFF_TOTAL} -eq 0 ]]; then
  RUFF_RESULT="PASS"
else
  RUFF_RESULT="FAIL"
fi
echo "Ruff: ${RUFF_RESULT} — issues: ${RUFF_TOTAL}, rules: ${RUFF_UNIQUE}"
...
# Consider Pylint PASS if it ran without critical errors
if [[ ${PYLINT_STATUS} -eq 0 ]]; then
  PYLINT_RESULT="PASS"
else
  PYLINT_RESULT="FAIL"
fi
echo "Pylint: ${PYLINT_RESULT} — messages: ${PYLINT_MSGS}, score: ${PYLINT_SCORE}"
...
# Exit behavior
# -------------------------------
# Exit non-zero only if Pylint fails critically
